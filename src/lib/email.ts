...
// Generate HTML email for insight report request
function generateInsightRequestEmailHTML(
  data: InsightRequestFormData,
  logoData?: LogoData
): string {
  return `
    <!DOCTYPE html>
    ...
    <head>
      <meta charset="UTF-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>New Insight Report Request</title>
      <style>
        body {
          font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
          line-height: 1.6;
    ...
    <body>
      <div class="container">
        <div class="header">
          ...
          <div class="subtitle">New Insight Report Request</div>
        </div>

        <div class="report-badge">
          <div class="report-badge-label">Requested Report</div>
          <div class="report-badge-value">${data.insightTitle}</div>
        </div>
        ...
        <div class="footer">
          ...
          <p>This request was submitted from the Onterra Capital insights page.</p>
        </div>
      </div>
    </body>
    </html>
...
// Generate plain text email for insight request
function generateInsightRequestEmailText(
  data: InsightRequestFormData
): string {
  return `
New Insight Report Request - Onterra Capital
...
---
This request was submitted from the Onterra Capital insights page.
  `.trim();
}

// Send insight request email
export async function sendInsightRequestEmail(
  data: InsightRequestFormData,
  logoData?: LogoData
): Promise<void> {
  try {
    ...
    const mailOptions = {
      from: `"Onterra Capital Insights" <${fromEmail}>`,
      to: toEmail,
      subject: `Insight Report Request: ${data.insightTitle} — from ${data.name}`,
      text: generateInsightRequestEmailText(data),
      html: generateInsightRequestEmailHTML(data, logoData),
    };
